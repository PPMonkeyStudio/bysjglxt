function Initialization_RecordProgress_4_Teacher() {
	var banner_RecordProgress_4_Teacher = document
			.getElementById("banner_RecordProgress_4_Teacher");
	banner_RecordProgress_4_Teacher.click();
}
function RecordProgress_4_Teacher() {

	document.getElementById("GraduationProjectTitle").innerHTML = '指导老师填写进展情况意见（完善阶段）';
	/*
	 * 
	 */
	var xhr = false;
	...
	xhr.onreadystatechange = function() {
		...
		if (xhr.readyState == 4) {
			if (xhr.status == 200) {
				...
				textarea_0.id = 'record_hou_progress_id';
				...
				h4.innerHTML = '进展情况意见（完善阶段）：';
				tab.appendChild(h4);
				var textarea_1 = document.createElement("textarea");
				textarea_1.id = "record_progress_opinion_4";
				textarea_1.className = 'form-control';
				textarea_1.style = "margin:10px 0 50px 0;resize: none;height:200px;"
				if (recordprocess_4.record_progress_opinion != null) {
					textarea_1.innerHTML = recordprocess_4.record_progress_opinion;
				} else {
				...
				aSave.href='####';
				aSave.onclick = Save_RecordProgress_4_Teacher
				aSave.text='提交更新';
				...
				if ('-1' == k) {
					if ("指导老师填写进展情况意见（完善阶段）" != current_processDefinitionName) {
