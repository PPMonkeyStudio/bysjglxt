function Initialization_examination_formal_leader() {

	var banner_examination_formal_leader = document
			.getElementById("banner_examination_formal_leader");
	banner_examination_formal_leader.click();
}

function examination_formal_leader() {
	document.getElementById("GraduationProjectTitle").innerHTML = '领导小组长填写形式审查表(核查)';
	/*
	 * 
	 */
	var xhr = false;
	...
	xhr.onreadystatechange = function() {
		...
		if (xhr.readyState == 4) {
			if (xhr.status == 200) {
				...
				aSave.href='####';
				aSave.onclick = Save_examination_formal_leader
				aSave.text='提交更新';
