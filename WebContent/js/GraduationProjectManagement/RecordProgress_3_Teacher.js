function Initialization_RecordProgress_3_Teacher() {
	var banner_RecordProgress_3_Teacher = document
			.getElementById("banner_RecordProgress_3_Teacher");
	banner_RecordProgress_3_Teacher.click();
}

function RecordProgress_3_Teacher() {
	document.getElementById("GraduationProjectTitle").innerHTML = '指导老师填写进展情况意见（中期自查阶段）';
	...
	xhr.onreadystatechange = function() {
		...
		if (xhr.readyState == 4) {
			if (xhr.status == 200) {
				...
				h4.innerHTML = '进展情况意见（中期自查阶段）：';
				tab.appendChild(h4);
				var textarea_1 = document.createElement("textarea");
				textarea_1.id = "record_progress_opinion_3";
				textarea_1.className = 'form-control';
				textarea_1.style = "margin:10px 0 50px 0;resize: none;height:200px;"
				if (recordprocess_3.record_progress_opinion != null) {
					textarea_1.innerHTML = recordprocess_3.record_progress_opinion;
				} else {
				...
				aSave.href='####';
				aSave.onclick = Save_RecordProgress_3_Teacher
				aSave.text='提交更新';
				...
				if ('-1' == k) {
					if ("指导老师填写进展情况意见（中期自查阶段）" != current_processDefinitionName) {
